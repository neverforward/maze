...
mc.world.afterEvents.chatSend.subscribe((e) => {
    ...
    if (cmd.length > 0 && cmd[0] == '#maze') {
        ...
        function createMaze(width = 21, height = 21) {
        ...
        if (cmd.length == 4) {
            if (cmd[3].toLocaleLowerCase() == 'true')
                path = true;
            else if (cmd[3].toLocaleLowerCase() == 'false')
                path = false;
            else {
        ...
        const x = e.sender.location.x, y = e.sender.location.y, z = e.sender.location.z;
